...
> Windows PowerShell 用户请使用 `curl.exe` 而不是 `curl`（PowerShell 中 `curl` 是 Invoke-WebRequest 的别名）。
...
下面以 **门店B（国贸店）紧急向门店A（望京店）调拨 15 把雨伞** 为例演示完整流程。

### 1. 门店B店员提交调拨申请

```bash
curl.exe -s -X POST http://localhost:3000/api/allocations ^
  -H "Content-Type: application/json" ^
  -d "{\"sourceStore\":\"store_a\",\"targetStore\":\"store_b\",\"product\":\"p_umbrella\",\"qty\":15,\"reason\":\"突降暴雨，门店B雨伞售罄，顾客大量需求\",\"operator\":\"u_store_b\"}"
```
...
```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_002/review ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_warehouse\"}"
```
...
```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_002/approve ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_manager\"}"
```
...
```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_002/ship ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_warehouse\"}"
```
...
curl.exe -s -X POST http://localhost:3000/api/allocations ^
  -H "Content-Type: application/json" ^
  -d "{\"sourceStore\":\"store_c\",\"targetStore\":\"store_b\",\"product\":\"p_water\",\"qty\":10,\"reason\":\"测试未复核审批\",\"operator\":\"u_store_b\"}"
```

假设返回 ID 为 `alloc_003`，直接审批：

```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_003/approve ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_manager\"}"
```
...
curl.exe -s -X POST http://localhost:3000/api/allocations ^
  -H "Content-Type: application/json" ^
  -d "{\"sourceStore\":\"store_a\",\"targetStore\":\"store_b\",\"product\":\"p_umbrella\",\"qty\":15,\"reason\":\"占满可用库存\",\"operator\":\"u_store_b\"}"
```

假设 ID 为 `alloc_004`，复核并审批：

```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_004/review ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_warehouse\"}"

curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_004/approve ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_manager\"}"
```
...
curl.exe -s -X POST http://localhost:3000/api/allocations ^
  -H "Content-Type: application/json" ^
  -d "{\"sourceStore\":\"store_a\",\"targetStore\":\"store_b\",\"product\":\"p_umbrella\",\"qty\":20,\"reason\":\"测试库存被占用\",\"operator\":\"u_store_b\"}"
```

假设 ID 为 `alloc_005`，复核时会失败：

```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_005/review ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_warehouse\"}"
```
...
curl.exe -s -X POST http://localhost:3000/api/allocations ^
  -H "Content-Type: application/json" ^
  -d "{\"sourceStore\":\"store_c\",\"targetStore\":\"store_b\",\"product\":\"p_mask\",\"qty\":10,\"reason\":\"测试撤回后出库\",\"operator\":\"u_store_b\"}"
```

假设 ID 为 `alloc_006`：

```bash
curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_006/review ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_warehouse\"}"

curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_006/approve ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_manager\"}"

curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_006/withdraw ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_store_b\",\"remark\":\"不需要了\"}"

curl.exe -s -X POST http://localhost:3000/api/allocations/alloc_006/ship ^
  -H "Content-Type: application/json" ^
  -d "{\"operator\":\"u_warehouse\"}"
```
