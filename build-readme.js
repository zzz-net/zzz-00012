...
const p = (url, json) =>
  `curl.exe --% -s -X POST ${url} -H "Content-Type: application/json" -d "${json.replace(/"/g, '\\"')}"`;

const content = `# 门店应急调拨 JSON API
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations', '{"sourceStore":"store_a","targetStore":"store_b","product":"p_umbrella","qty":15,"reason":"突降暴雨，门店B雨伞售罄，顾客大量需求","operator":"u_store_b"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_002/review', '{"operator":"u_warehouse"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_002/approve', '{"operator":"u_manager"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_002/ship', '{"operator":"u_warehouse"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_002/review', '{"operator":"u_store_b"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations', '{"sourceStore":"store_c","targetStore":"store_b","product":"p_water","qty":10,"reason":"测试未复核审批","operator":"u_store_b"}')}
\`\`\`

假设返回 ID 为 \`alloc_003\`，直接审批：

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_003/approve', '{"operator":"u_manager"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations', '{"sourceStore":"store_a","targetStore":"store_b","product":"p_umbrella","qty":15,"reason":"占满可用库存","operator":"u_store_b"}')}
\`\`\`

假设 ID 为 \`alloc_004\`，复核并审批：

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_004/review', '{"operator":"u_warehouse"}')}
\`\`\`

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_004/approve', '{"operator":"u_manager"}')}
\`\`\`

此时门店A雨伞可用库存接近 0。再新建一个超过可用量的申请：

\`\`\`powershell
${p('http://localhost:3000/api/allocations', '{"sourceStore":"store_a","targetStore":"store_b","product":"p_umbrella","qty":20,"reason":"测试库存被占用","operator":"u_store_b"}')}
\`\`\`

假设 ID 为 \`alloc_005\`，复核时会失败：

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_005/review', '{"operator":"u_warehouse"}')}
\`\`\`
...
\`\`\`powershell
${p('http://localhost:3000/api/allocations', '{"sourceStore":"store_c","targetStore":"store_b","product":"p_mask","qty":10,"reason":"测试撤回后出库","operator":"u_store_b"}')}
\`\`\`

假设 ID 为 \`alloc_006\`，依次执行：

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_006/review', '{"operator":"u_warehouse"}')}
\`\`\`

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_006/approve', '{"operator":"u_manager"}')}
\`\`\`

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_006/withdraw', '{"operator":"u_store_b","remark":"不需要了"}')}
\`\`\`

\`\`\`powershell
${p('http://localhost:3000/api/allocations/alloc_006/ship', '{"operator":"u_warehouse"}')}
\`\`\`
...
console.log(`README.md 写入成功，大小: ${stat.size} 字节`);

let ok = true;
const caretLines = content.split('\n').filter(l => l.trimEnd().endsWith('^'));
if (caretLines.length > 0) {
  console.log(`❌ 发现 ${caretLines.length} 行仍包含 ^ 续行符（CMD 语法）`);
  ok = false;
} else {
  console.log('✅ 未发现 ^ 续行符');
}

const postLines = content.split('\n').filter(l => l.includes('curl.exe') && l.includes('-X POST'));
const badPost = postLines.filter(l => !l.includes('--%'));
if (badPost.length > 0) {
  console.log(`❌ 发现 ${badPost.length} 行 POST 示例缺少 --%`);
  ok = false;
} else if (postLines.length > 0) {
  console.log(`✅ 全部 ${postLines.length} 个 POST 示例均使用 --% 停止解析`);
}

const badQuote = postLines.filter(l => !l.includes('-d "{\\"'));
if (badQuote.length > 0) {
  console.log(`❌ 发现 ${badQuote.length} 行 POST 示例未使用 \\" 转义双引号`);
  ok = false;
} else if (postLines.length > 0) {
  console.log('✅ POST 示例均使用正确的 JSON 转义格式');
}
